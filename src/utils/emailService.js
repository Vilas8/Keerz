export const sendEmails = async (leadData, ticketImageBase64 = null) => {
  try {
    ...
    // 2. Load SMTP.js dynamically if not already loaded in window
    if (!window.Email) {
      await new Promise((resolve, reject) => {
        const script = document.createElement('script');
        script.src = 'https://smtpjs.com/v3/smtp.js';
        script.onload = resolve;
        script.onerror = () => reject(new Error('Failed to load SMTP.js library.'));
    ...
    // 4. Send Email to Candidate (with Ticket Attachment)
    const candidatePromise = window.Email.send({
      Host: "smtp.gmail.com",
      Username: user,
      Password: pass,
      To: leadData.email,
    ...
    // 5. Send Email to Admin (with Details)
    const adminPromise = window.Email.send({
      Host: "smtp.gmail.com",
      Username: user,
      Password: pass,
    ...
    const [candidateRes, adminRes] = await Promise.all([candidatePromise, adminPromise]);
